...
foreach($get_header as $valeheder){
   ...
   $get_mainfooter_jeson = $valeheder['footer_copyright'];

$raw = $valeheder['footer_copyright'];

// Remove the outer double quotes
$raw = trim($raw, "\"");

// Remove HTML escape slashes
$raw = stripslashes($raw);

// Now split the 3 items inside brackets
preg_match_all('/\[(.*)\]/sU', "[" . $raw . "]", $matches);

if (!empty($matches[1][0])) {
    // split by ",""," pattern
    $parts = preg_split('/",""\s*,\s*"/', $matches[1][0]);

    // clean extra quotes
    $get_mainfooter_raw = array_map(function($v){
        return trim($v, "\"");
    }, $parts);
...
foreach($get_headerhome as $valehederhome){
    ...
    $get_numemail = json_decode($valehederhome['head_numberbox']);
}
?>
        ...
        <form role="form" method="post" enctype="multipart/form-data" action="">
            ...
            <div class="col-md-12">
              <div class="form-group">
                ...
                <textarea class="textarea" name="ft-main-emailtitle" placeholder="Place some text here" style="width: 100%; height: 200px; font-size: 14px; line-height: 18px; border: 1px solid #dddddd; padding: 10px;"><?php echo htmlspecialchars($get_mainfooter_raw[0]); ?></textarea>
              </div>
             <!-- <div class="form-group">
                  <label>Email Signup Footer Content</label>
                <textarea class="textarea" name="ft-main-emailbtn" placeholder="Place some text here" style="width: 100%; height: 200px; font-size: 14px; line-height: 18px; border: 1px solid #dddddd; padding: 10px;"><?php echo $get_mainfooter_raw[1]; ?></textarea>
              </div>-->
            </div>
            ...
            <div class="col-md-12">
              <div class="form-group">
                ...
                <textarea class="textarea" name="ft-main-copyrihgt" placeholder="Place some text here" style="width: 100%; height: 200px; font-size: 14px; line-height: 18px; border: 1px solid #dddddd; padding: 10px;"><?php echo htmlspecialchars($get_mainfooter_raw[1]); ?></textarea>
              </div>
            </div>
            <div class="col-md-12">
              <div class="form-group">
                  <label>Whatsapp No</label>
                <input type="text" class="form-control" placeholder="Number" name="head-number" value="<?php echo $get_numemail[0]; ?>">
              </div>
            </div>
            <div class="col-md-12">
              <div class="form-group">
                  <label>Email ID</label>
                <input type="text" class="form-control" placeholder="Email Id" name="head-email" value="<?php echo $get_numemail[1]; ?>">
              </div>
            </div>
            <div class="col-md-12">
              <div class="form-group">
