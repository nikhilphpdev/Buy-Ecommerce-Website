...
if(isset($_GET['delete']) && isset($_GET['id']) && isset($_GET['seoid'])){
    ...
    if($delete_seovale == true){
      echo "<script>alert('Successfully Deleted.');window.location.href='$url/admin-manager/category/';</script>";
  	}else{
...
if(isset($_GET['hideid']) && isset($_GET['hide'])){
  ...
  if($updateset == true){
      $gethidsval = $_GET['hideid'];
      $getsubidsvalue = "SELECT * FROM product_categories WHERE prd_cat_prent_categ='$gethidsval'";
      $querygetcatgouy = $contdb->query($getsubidsvalue);
      if($querygetcatgouy->num_rows > 0){
        while($rowgetcatsub = $querygetcatgouy->fetch_array()){
            $getidsvales = $rowgetcatsub['id'];
            $updatesesub = "id='$getidsvales'";
            $updatevale = UpdateAllDataFileds("product_categories",$update_datafiled,$updatesesub);
        }   
      }
          ...
          <div class="card card-primary">
            ...
            <div class="card-header">
              ...
              <h3 class="card-title">All Categories</h3>
                ...
                <a href="<?php echo $url; ?>/admin-manager/addnew-category/" class="btn btn-sm btn-success float-right">Add New Category</a>
