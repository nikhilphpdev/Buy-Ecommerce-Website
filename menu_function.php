...
function showmenufootercontact(){
  ...
  echo '<li><span><i class="fa fa-phone"></i></span><a href="tel:7185031339" target="blank">+1-718-503-1339</a></li>

          <li><span><i class="fa fa-envelope-o"></i> </span> <a href="mailto:info@jioitservices.com" target="blank">info@jioitservices.com</a></li>
...
function single_product_name($showdataid){
  ...
  $shresingid = "SELECT * FROM all_product WHERE product_auto_id='$showdataid'";
  ...
  while($rowproduid = mysqli_fetch_array($querysharename)){
    ...
    $singleprodidshar = $rowproduid['product_name'];
            ...
            <div class='col-lg-7 col-md-7 col-sm-7'>
                ...
                <div class='pro-description'>
                    ...
                    if(isset($_SESSION['filter-product'])){
                      $trem_product_id = $_SESSION['filter-product'][0];
                      $trem_get_ids = $_SESSION['filter-product'][1];
                      if($trem_product_id == $pId){
                        $get_quntity_value = "SELECT * FROM product_attbut_vartarry WHERE prot_trm_id='$trem_get_ids' AND prot_trm_prodtid='$editviewprodcolor'";
                        $get_qunityvale = $conn->query($get_quntity_value);
                        while($row_qunityvale_dat = $get_qunityvale->fetch_array()){
                          $get_Qunity_val = $row_qunityvale_dat['prot_trm_quantity'];
                        }
                        $get_cart_qunityval = "SELECT * FROM cart_user WHERE cart_prdo_sizename='$trem_get_ids' AND cart_prdo_auto_id='$editviewprodautoid'";
                        $query_cart_cechking = $conn->query($get_cart_qunityval);
                        if($query_cart_cechking > 0){
                          while($get_qunity_vale = $query_cart_cechking->fetch_array()){
                            $get_qunity_valuedat = $get_qunity_vale['cart_prdo_qutity'];
                          }
                          if($get_Qunity_val == ""){
                            $qunity_cart_notincre = "1";
                          }else{
                            if($get_Qunity_val > $get_qunity_valuedat){
                              $qunity_cart_notincre = "1";
                            }else{
                    ...
                    include_once('variations-template/price.php');
                    include_once('variations-template/selection.php');
                    /*-- Out Of Stock Statr --*/
              ...
              if($tagtrimval == ""){}else{
                echo "<a href='$url/product-tag/$catsliedh' rel='$tagtrimval'>$tagtrimval</a><span>, </span>";
                    ...
                    $prodtvenderdetalsfirst = "SELECT * FROM vendor WHERE vendor_auto='$editviewprodauvid' LIMIT 1";
                      ...
                      while($rowprodvidfirst = mysqli_fetch_array($queryprovidfirst)){
                        ...
                        $shppied_from = $rowprodvidfirst['vendor_country'];
                      }

                      $get_about = "SELECT * FROM about_me WHERE uid='$prodvcompautofirst'";
                      $query_about = $conn->query($get_about);
                      while($row_quyer = $query_about->fetch_array()){
                        $ge_aboutvendor = $row_quyer['about_content'];
                      }

                      $shpiingdataval = "SELECT * FROM termsCondition WHERE uid='$prodvcompautofirst'";
                      $query_shppingdat = $conn->query($shpiingdataval);
                      while($rowget_shppingdat = $query_shppingdat->fetch_array()){
                        $shppingvaledat = $rowget_shppingdat['terms'];
                      }
                      ...
                      echo "<span class='tagged_as'><span class='bold-data'>Product will ship from: </span> $shppied_from</span>";
                ...
                echo "<div class='item-info'>
                      ...
                      <ul class='nav nav-tabs' id='myTab' role='tablist'>
                          ...
                          <li class='nav-item'> <a class='nav-link' id='ds-5' data-toggle='tab' href='#ds-t5' role='tab'>About Creator</a></li>

                          <li class='nav-item'> <a class='nav-link' id='ds-6' data-toggle='tab' href='#ds-t6' role='tab'>Shipping and Returns</a></li>
                      ...
                      <div class='tab-content' id='myTabContent'>
                          ...
                          <div class='tab-pane fade in' id='ds-t5' role='tabpanel' aria-labelledby='ds-5'>
                              <div class='dc-list'>
                                  <h4>About Creator</h4>
                                  $ge_aboutvendor
                              </div>
                          ...
                          <div class='tab-pane fade in' id='ds-t6' role='tabpanel' aria-labelledby='ds-6'>
                              <div class='dc-list'>
...
function showrelateproduct($showdataid,$productsingname){
  ...
  global $url;
	...
	$showallreletprodt = "SELECT * FROM all_product WHERE product_status='1'";
 ...
 $attbutvalcolor = $rowshoprodt['product_color'];
		...
		if($singleprodtcatidall == $singleprodtcatid){
			...
			if($productsingname == $rowshoprodt['product_page_name']){}else{
				...
				echo "<div class='item' $pId>
                    ...
                    //echo $attbutvalcolor;
                    if($attbutvalcolor == "" || $attbutvalcolor == "0"){

                      if($rowshoprodt['product_sale_price'] == ""){
                             echo "<div class='item-price'><div class='pric_reg'><span>$</span>".number_format($rowshoprodt['product_regular_price'], 2)."</div></div>";
                          }else{
                            echo "<div class='item-price'><div class='pric_dis'><span>$</span>".number_format($rowshoprodt['product_regular_price'], 2)."</div> <div class='pric_reg'><span>$</span>".$rowshoprodt['product_sale_price']."</div>";
                            echo "</div>";
                        }
                        if($stock == "0"){
                        echo "<div class='item-cart'><button class='alert alert-danger unavailable'>Out of Stock</button></div>";
                        ...
                        echo "<div class='item-cart'><button pid=$pId class='add-cart addToCart addtoCart'>Add to Cart</button></div>";
                      }

                    }else{

                      $get_vertionprice = "SELECT * FROM product_attbut_vartarry WHERE prot_trm_prodtid='$attbutvalcolor'";
                      ...
                      if($sale_price == "0" || $sale_price == ""){
                        $sale_min = "SELECT MIN(prot_trm_regulprc) FROM product_attbut_vartarry WHERE prot_trm_prodtid='$attbutvalcolor'";
                        $sale_resultmin = mysqli_query($conn, $sale_min);
                        $sale_rowmin = mysqli_fetch_array($sale_resultmin);
                        $sale_minvalmin = $sale_rowmin[0];

                        $sale_max = "SELECT MAX(prot_trm_regulprc) FROM product_attbut_vartarry WHERE prot_trm_prodtid='$attbutvalcolor'";
                        $sale_resultmax = mysqli_query($conn, $sale_max);
                        $sale_rowmax = mysqli_fetch_array($sale_resultmax);
                        $sale_minvalmax = $sale_rowmax[0];
                        if($sale_minvalmin == $sale_minvalmax){
                          echo "<div class='item-price'><div class='pric_reg'><span>$</span>".number_format($get_total_regulor, 2)."</div></div>";
                        }else{
                          echo "<div class='item-price'><div class='pric_reg'><span>$</span>".number_format($sale_minvalmin, 2)."</div> - <div class='pric_reg'><span>$</span>".number_format($sale_minvalmax, 2)."</div></div>";
                        }
                      }else{
                        ...
                        $sale_max = "SELECT MAX(prot_trm_saleprc) FROM product_attbut_vartarry WHERE prot_trm_prodtid='$attbutvalcolor'";
                        $sale_resultmax = mysqli_query($conn, $sale_max);
                        $sale_rowmax = mysqli_fetch_array($sale_resultmax);
                        $sale_minvalmax = $sale_rowmax[0];
                          if($sale_minvalmin == $sale_minvalmax){
                            echo "<div class='item-price'><div class='pric_dis'><span>$</span>".number_format($get_total_regulor, 2)."</div> <div class='pric_reg'><span>$</span>".number_format($sale_minvalmin, 2)."</div></div>";
                          }else{
                            echo "<div class='item-price'><div class='pric_reg'><span>$</span>".number_format($sale_minvalmin, 2)." - <span>$</span>".number_format($sale_minvalmax, 2)."</div></div>";
                          }
                      }
                      if(in_array("", $quntity)){
                        $qunityvale = "1";
                  ...
                  <div class="item-txt">
                    ...
                    if($regPrice == "0" || $regPrice == ""){
                      ...
                      if($row['product_sale_price'] == ""){
                          ...
                          echo "<div class='item-price'><div class='pric_dis'><span>$</span>".number_format($row['product_regular_price'], 2)."</div> <div class='pric_reg'><span>$</span>".number_format($row['product_sale_price'], 2)."</div>";
...
function NewsletterSubscriberProduct(){
  global $conn;
  global $url;

  $sql = "SELECT * FROM all_product WHERE product_status='1' ORDER BY RAND() DESC LIMIT 4";
  $qry = mysqli_query($conn,$sql);
  $count = 1;
  $numRows = mysqli_num_rows($qry);
  if(mysqli_num_rows($qry) !=  0 ){
                ...
                echo '<a href="'.$url.'/'.$row['product_page_name'].'">
                  ...
                  <div class="item-txt">
                    ...
                    if($regPrice == "0" || $regPrice == ""){
                      ...
                      $sale_minvalmax = $sale_rowmax[0];
                        if($sale_minvalmin == $sale_minvalmax){
                          echo "<div class='item-price'><div class='pric_reg'><span>$</span>".number_format($sale_minvalmin, 2)."</div></div>";
                        }else{
                          echo "<div class='item-price'><div class='pric_reg'><span>$</span>".number_format($sale_minvalmin, 2)." - <span>$</span>".number_format($sale_minvalmax, 2)."</div></div>";
                        }
                      }
...
function insertNewsletterSubscriberProduct($get_emailid,$url){
  global $conn;
  $date = date('m/d/Y');
  $time = date('H:i:s');
  global $ip;

  $_chking_emailid = "SELECT * FROM newslettersubscribertabl WHERE newlat_emailid='$get_emailid'";
  $query_chkingemal = $conn->query($_chking_emailid);
  if($query_chkingemal->num_rows > 0){
    return false;
  }else{
    $insert_news_data = "INSERT INTO newslettersubscribertabl(newlat_emailid,newlat_ip,newlat_date,newlat_time,newlat_url)VALUES('$get_emailid','$ip','$date','$time','$url')";
    $query_leadternst = $conn->query($insert_news_data);
    return true;
  }
}
                        ...
                        <table align='center' border='0' cellpadding='0' cellspacing='0' width='600' style='border: 1px solid #cccccc; border-collapse: collapse;'>
                            ...
                            <tr>
                                ...
                                <td align='center' bgcolor='#0fa8ae' style='padding: 40px 0 30px 0; color: #FFF; font-size: 28px; font-weight: bold; font-family: Arial, sans-serif;'>
                                    ...
                                    <img src='https://buyjee.com/assets/images/logo.png' alt='Creating Email Magic' width='300' height='230' style='display: block;' />
                                    ...
                                    <table border='0' cellpadding='0' cellspacing='0' width='100%'>
                                        ...
                                        <tr>
                                            ...
                                            <td style='padding: 20px 0 30px 0; color: #153643; font-family: Arial, sans-serif; font-size: 16px; line-height: 30px;'>
                                                ...
                                                Please  <a href='https://buyjee.com/resetpassword?token=".$token."&auto=".$auto_id."&email=".$email_vale."'>click here</a> to reset your password. <br/><br/><br/>
      ...
      echo '<li>
                ...
                echo '<a href="'.$url.'/'.$row['product_page_name'].'">
                  ...
                  <div class="item-txt">
                    ...
                    // price set
                    if($row['product_color'] == "" || $row['product_color'] == "0"){
                      ...
                      $product_colorval = $row['product_color'];
                      
                      $get_min_sale_price = "SELECT MIN(prot_trm_saleprc) FROM product_attbut_vartarry WHERE prot_trm_prodtid='$product_colorval'";
                      ...
                      $get_max_sale_price = "SELECT MAX(prot_trm_saleprc) FROM product_attbut_vartarry WHERE prot_trm_prodtid='$product_colorval'";
                      ...
                      $get_min_regul_price = "SELECT MIN(prot_trm_regulprc) FROM product_attbut_vartarry WHERE prot_trm_prodtid='$product_colorval'";
                      ...
                      $get_max_reular_price = "SELECT MAX(prot_trm_regulprc) FROM product_attbut_vartarry WHERE prot_trm_prodtid='$product_colorval'";
                      ...
                      if($row_set_min_sale_pric == "0" || $row_set_min_sale_pric == ""){

                        if($row_set_min_regul_pric == $row_set_max_reular_pric){
                          echo "<div class='item-price'><div class='pric_reg'><span>$</span>".number_format($row_set_min_regul_pric, 2)."</div></div>";
                        }else{
                          echo "<div class='item-price'><div class='pric_reg'><span>$</span>".number_format($row_set_min_regul_pric, 2)." - ".number_format($row_set_max_reular_pric, 2)."</div></div>";
                        }

                      }else{

                        if($row_set_min_sale_pric == $row_set_max_sale_pric){
                          echo "<div class='item-price'><div class='pric_reg'><span>$</span>".number_format($row_set_min_sale_pric, 2)."</div></div>";
                        }else{
                          echo "<div class='item-price'><div class='pric_reg'><span>$</span>".number_format($row_set_min_sale_pric, 2)." - ".number_format($row_set_max_sale_pric, 2)."</div></div>";
                        }
                      }

                      $get_vertionvale = "SELECT * FROM product_attbut_vartarry WHERE prot_trm_prodtid='$product_colorval'";
                      $get_stock_vale = $conn->query($get_vertionvale);
                      while($row_valdata_stock = $get_stock_vale->fetch_array()){
                        $get_stock_data[] = $row_valdata_stock['prot_trm_quantity'];
                        $quntityadd += $row_valdata_stock['prot_trm_quantity'];
                      }
                      if(in_array("", $get_stock_data)){
                        $qunityvale = "1";
                      }else{
                        if($quntityadd == "0"){
                          $qunityvale = "0";
...
function login_checking_code($login_name,$login_password){
  ...
  $cehck_val = mysqli_query($conn,$checkpassword);
  if(mysqli_num_rows($cehck_val)){
      ...
      $nuvale = mysqli_query($conn,$get_auto_id);
      ...
      $queryupdatecart = $conn->query($update_productval);
...
function gust_user_sessiondata($email_idgust){
  global $conn;
  global $ip;
  $date = date('m/d/Y');
  $time = date('H:i:s');
  ...
  /*$gust_user_chking = "SELECT * FROM userlogntable WHERE user_email='$email_idgust' AND user_type='customer'";
  $query_gust_cking = $conn->query($gust_user_chking);
  if($query_gust_cking->num_rows > 0){*/
  /*  while($get_customer_login = $query_gust_cking->fetch_array()){
      $get_customer_id = $get_customer_login['user_auto'];
      $_SESSION['customersessionlogin']=$get_customer_id;
    ...
    return true;
  /*}else{
    $insert_login_detal = "INSERT INTO userlogntable(user_first_name,user_email,user_lastname,user_password,user_session_id,user_type,user_status,user_auto)VALUES('Guest','$email_idgust','User','Guest','0','customer','0','$create_useremailid')";
    $query_loginset_user = $conn->query($insert_login_detal);

    $insert_user_customer = "INSERT INTO customer(customer_ui_id,customer_img,customer_date,customer_time,customer_active,customer_type)VALUES('$create_useremailid','0','$date','$time','1','Guest')";
    $insert_customer_vale = $conn->query($insert_user_customer);
    $_SESSION['customersessionlogin']=$create_useremailid;
    $_SESSION['gust_customer']="GuestCoustomer";
    $update_productval = "UPDATE cart_user SET cart_userid='$create_useremailid' WHERE cart_user_ip='$ip' AND cart_status='0'";
    $queryupdatecart = $conn->query($update_productval);
    return true;
  }*/
}
