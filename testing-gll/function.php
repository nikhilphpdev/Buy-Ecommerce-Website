...
function view_make_login(){
	global $con;
	$url = "https://www.gallerylala.com/testing-gll";
...
if(isset($_POST['login_tab'])){
	...
	$check_pass_query = mysqli_query($con, "SELECT * FROM login_table WHERE login_email='$email_check' AND login_password='$passwd_check'");
	if(mysqli_num_rows($check_pass_query)){
		session_start();
		echo "<script>window.open('$url/index/','_Self');</script>";
...
function threepidname(){
    global $con;
    
    $getpidname = "SELECT * FROM three_link";
    $queryvale = mysqli_query($con,$getpidname);
    while($rowvalepid = mysqli_fetch_array($queryvale)){
        $clintid = $rowvalepid['three_clint_pid'];
        $ownid = $rowvalepid['three_own_id'];
        echo "<option value='$clintid|$ownid'>$clintid</option>";
    }
}
...
function all_pid(){
	...
	while($row_pid_fecth = mysqli_fetch_array($pid_get)){
		...
		$explodat = explode('|', $pid_name);
		$exploclint = $explodat[0];
		echo "<option value='$pid_name|$pid_priy_key'>$exploclint</option>";
	}
}
...
if(isset($_POST['proj_add_clint'])){
	...
	if(mysqli_num_rows($pid_check_data)){
		echo "<script>alert('Please Try Again. This id already use');window.location.href='$url/client_add_project.php'</script>";
	}else{
	...
	if($inset_projct_data == true){
		echo "<script>alert('Successfully add project');window.location.href='$url/client_add_project.php';</script>";
	}else{
		echo "<script>alert('Please Try Again');window.location.href='$url/client_add_project.php';</script>";
	}
	}
}
...
if(isset($_POST['alt_projt_btn'])){
	...
	$explot_pid_key = $explode_pid_data[2];
	...
	$loivender = $_POST['loi_p_l'];

	$pr_l_clt_lin_data = mysqli_query($con, "SELECT * FROM client_proj_detail WHERE client_p_d_clint_id='$expt_clt_key' AND client_p_d_auto_id='$expt_clt_auto'");
	while($row_p_l_link = mysqli_fetch_array($pr_l_clt_lin_data)){
		$p_l_clnt_link = $row_p_l_link['client_p_d_link'];
	}
	$make_ved_id = $explot_pid_name.$exp_ve_p_l_name.$exp_ve_p_l_email.$expt_clt_auto.$newownpid;
	$rander_val_id = trim(str_shuffle($make_ved_id));
	$ved_id_limit_word = substr($rander_val_id,0,8);
	//die();
	$get_three_auto = "SELECT * FROM three_link WHERE three_clint_pid='$explot_pid_name'";
	$gwetpidval = mysqli_query($con,$get_three_auto);
	while($rowpiclnt = mysqli_fetch_array($gwetpidval)){
	    $sing_pid = $rowpiclnt['three_own_id'];
	}
	$full_link = "$url/vendor/?$ved_id_limit_word/?$sing_pid/&UID=YOUR_USER_ID";

	$insert_alt_projt = mysqli_query($con, "INSERT INTO vander_alt_project(vander_a_p_clint_name,vander_a_p_clint_key,vander_a_p_clint_auto,vander_a_p_v_pid_name,vander_a_p_v_pid_key,vander_a_p_quota,vander_a_p_loi,vander_a_p_date,vander_a_p_time,vander_a_p_status,vander_a_p_cpi,vander_a_p_ownpid,vander_a_p_cl_link,vander_a_p_vd_sid,vander_a_p_vd_pid,vander_a_p_full_link,vander_a_p_vd_name,vander_a_p_vd_email,vander_a_p_v_id,vander_a_p_v_auto,vander_a_p_thank,vander_a_p_termint,vander_a_p_overquta)VALUES('$expt_clt_name','$expt_clt_key','$expt_clt_auto','$explot_pid_name','$explot_pid_key','$alt_p_quta','$loivender','$date','$time','$alt_p_status','$alt_p_cpi','$sing_pid','$p_l_clnt_link','$ved_id_limit_word','$vend_p_l_pid','$full_link','$exp_ve_p_l_name','$exp_ve_p_l_email','$exp_ve_p_l_id','$exp_ve_p_l_auto','$thank_lint','$ternat_lint','$overqta_lint')");
	if($insert_alt_projt == true){
		echo "<script>alert('Successfully Alot');window.location.href='$url/lot_project.php?vid=$ved_id_limit_word';</script>";
	}else{
		echo "<script>alert('Please Try Again');window.location.href='$url/lot_project.php';</script>";
	}
}
...
if(isset($_POST['make_three_link'])){
	  ...
	  $clintexpld = explode('|', $clintname);
	  $clintnameval = $clintexpld[0];
	  $clintidval = $clintexpld[1];
	  $clintautoval = $clintexpld[2];
	  $rand_vale = rand(888,9999);
	  $code_vale = "WPS";
	  $ownid = $code_vale.$rand_vale;
	  $shpid = $ownid.$clintpid;
	  $stractid = str_shuffle($shpid);
	  $date = date('m/d/Y');
	  $time = date('H:i:s');
	  
	  $check_vale = "SELECT * FROM three_link WHERE three_clint_pid='$clintpid'";
	  $quweryvale = mysqli_query($con,$check_vale);
	  if(mysqli_num_rows($quweryvale)){
	      
	      $get_own_id = "SELECT * FROM three_link WHERE three_clint_pid='$clintpid'";
	      $get_valeuid = mysqli_query($con,$get_own_id);
	      while($rowvaledata = mysqli_fetch_array($get_valeuid)){
	          $ownuid = $rowvaledata['three_own_id'];
	          $sectycuid = $rowvaledata['three_sectr_id'];
	          
	      echo "<script>window.location.href='https://www.gallerylala.com/testing-gll/three_links/?$sectycuid/?$ownuid/&value=1/';</script>";
	      }
	      
	  }else{
	      $insertthree = "INSERT INTO three_link(three_clint_name,three_clint_id,three_clint_auto,three_clint_pid,three_own_id,three_sectr_id,three_status,three_date,three_time)VALUES('$clintnameval','$clintidval','$clintautoval','$clintpid','$ownid','$stractid','1','$date','$time')";
	      $insertdata = mysqli_query($con,$insertthree);
	      if($insertdata == true){
	          echo "<script>window.location.href='https://www.gallerylala.com/testing-gll/three_links/?$stractid/?$ownid/&value=1/';</script>";
	      }else{
	          echo "<script>window.location.href='https://www.gallerylala.com/testing-gll/three_links/?error/?2/&value=0';</script>";
	      }
	  }
}
?>