...
$own_vid_url = $explode_vale[2];
$explodataown = explode('/',$own_vid_url);
$own_vid = $explodataown[0];
$vendor_uid = $_GET['UID'];
$ip = getUserIP();
...
if(mysqli_num_rows($query_vale)){
    ...
    $cehck_quta = "SELECT COUNT(1) FROM project_status WHERE p_status_security_id='$auto_vid' AND p_status_own_id='$own_vid' AND p_status_url_use='0'";
    $chk_qutaquery = mysqli_query($con,$cehck_quta);
    $countcompvale = mysqli_fetch_array($chk_qutaquery);
    $countcomprow = $countcompvale[0];
    
    $get_qutavale = "SELECT * FROM vander_alt_project WHERE vander_a_p_ownpid='$own_vid' AND vander_a_p_vd_sid='$auto_vid' AND vander_a_p_quota <= '$countcomprow'";
    $getpriosyc = mysqli_query($con,$get_qutavale);
    
    if(mysqli_num_rows($getpriosyc)){
        echo "Over Quota This Study.";
    }else{
        $chk_Ip_username = "SELECT * FROM project_status WHERE p_status_end_ip='$ip' AND p_status_own_id='$own_vid'";
        $vendorquery = mysqli_query($con,$chk_Ip_username);
        if(mysqli_num_rows($vendorquery)){
            echo "This Ip Alreday use. Please Try Again.";
        }else{
            $get_uiddata = "SELECT * FROM project_status WHERE p_status_uid='$vendor_uid' AND p_status_own_id='$own_vid'";
            $uidquyery = mysqli_query($con,$get_uiddata);
            if(mysqli_num_rows($uidquyery)){
                echo "This UID Alreday use. Please Try Again.";
            }else{
